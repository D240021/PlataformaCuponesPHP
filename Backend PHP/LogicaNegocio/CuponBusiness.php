...
class CuponBusiness {
    ...
    public function crearCupon($cupon) {
        // Validaciones
        if (empty($cupon->codigo) || empty($cupon->nombre) || empty($cupon->precio) || empty($cupon->empresa_id) || empty($cupon->estado) || empty($cupon->imagen) || empty($cupon->tipo) || empty($cupon->categoria_id) || empty($cupon->fecha_inicio) || empty($cupon->fecha_vencimiento) || empty($cupon->fecha_creacion)) {
